                ...
                <div class="extra-food-wrap">
                    ...
                    @if (count($paidaddons) != 0)
                        <ul class="list-unstyled extra-food">
                            ...
                            <li>
                                <input type="checkbox" name="addons[]" class="Checkbox" value="{{$addons->id}}" price="{{$addons->price}}">
                                <p>{{$addons->name}} : <?php echo env('CURRENCY'); ?>{{number_format($addons->price, 2)}}</p>
                            </li>
                            @endforeach
                            </div>
                        </ul>
                    ...
                    <div class="pro-details-add-wrap">
                        <p class="pricing"><?php echo env('CURRENCY'); ?>{{number_format($getitem->item_price, 2)}}</p>
                        <p class="open-time"><i class="far fa-clock"></i> {{$getitem->delivery_time}}</p>
                        @if (Session::get('id'))
                            @if ($getitem->item_status == '1')
                               <?php
                                 $Date = date('Y-m-d');
                               $count = \App\Cart::where(['user_id'=>Session::get('id'),'item_id'=>$getitem->id])->whereRaw('DATE(created_at)',$Date)->count(); ?>
                             
                                @if($count>=2)
                                <!-- <button class="btn" style="display:none" onclick="AddtoCart('{{$getitem->id}}','{{Session::get('id')}}')">Add to Cart</button> -->
                                 @else
